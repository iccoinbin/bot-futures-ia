...
def build_features(df: pd.DataFrame, ema_fast: int, ema_slow: int, atr_period: int, adx_period: int, vwap_window: int) -> pd.DataFrame:
    out = df.copy()
    ...
    out["vwap"] = vwap(out["close"], out["volume"], vwap_window)
